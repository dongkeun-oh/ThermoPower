...
package LHe "Models of components with water/steam as working fluid"
...
<li><i>1 Oct 2003</i>
  ...
  package CryoHelium "ph_explicit cryogenic Helium model from NIST RefProp database"
    ...
    model SourcePressure "Pressure source for helium liquid/gas flows"
      ...
      parameter Medium.Temperature T = 4.2 "Nominal temperature" annotation(
        Dialog(enable = use_T and not use_in_T));
      parameter Medium.SpecificEnthalpy h = -1e3 "Nominal specific enthalpy" annotation(
  ...
  model SinkPressure "Pressure sink for helium liquid/gas flows"
    extends Icons.LHe.SourceP;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    type HydraulicResistance = Real(final quantity = "HydraulicResistance", final unit = "Pa/(kg/s)");
...
<li><i>1 Oct 2003</i>
  ...
  model SourceMassFlow "Flowrate source for helium liquid/gas flows"
    extends Icons.LHe.SourceW;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialPureSubstance "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>1 Oct 2003</i>
  ...
  model SinkMassFlow "Flowrate sink for helium liquid/gas flows"
    extends Icons.LHe.SourceW;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialPureSubstance "Medium model" annotation(
       choicesAllMatching = true);
    parameter Medium.MassFlowRate w0 = 0 "Nominal mass flowrate";
...
<li><i>18 Mar 2005</i>
  ...
  model PressDropLin "Linear pressure drop for helium liquid/gas flows"
...
<li><i>1 Oct 2003</i>
  ...
  model PressDrop "Pressure drop for helium liquid/gas flows"
    extends Icons.LHe.PressDrop;
...
<li><i>1 Oct 2003</i>
  ...
  model Header "Header with metal walls for helium liquid/gas flows"
    ...
    parameter Medium.AbsolutePressure pstart "Pressure start value" annotation(
      Dialog(tab = "Initialisation"));
    parameter Medium.SpecificEnthalpy hstart = if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Liquid then -1e3 else if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Steam then 1e4 else 5e5 "Specific enthalpy start value" annotation(
      Dialog(tab = "Initialisation"));
...
// do nothing
    ...
    annotation(
      Icon(graphics),
      Documentation(info = "<HTML>
<p>This model describes a constant volume header with metal walls. The fluid can be liquid, gas, or a two-phase mixture.
...
<li><i>1 Oct 2003</i>
  ...
  model Mixer "Mixer with metal walls for helium liquid/gas flows"
...
// do nothing
    ...
    annotation(
      Placement(transformation(extent = {{80, -20}, {120, 20}}, rotation = 0)),
      Documentation(info = "<HTML>
<p>This model describes a constant volume mixer with metal walls. The fluid can be liquid, gas, or a two-phase mixture. The metal wall temperature and the heat transfer coefficient between the wall and the fluid are uniform. The wall is thermally insulated from the outside.
</HTML>", revisions = "<html>
<ul>
<li><i>23 May 2008</i>
...
// do nothing
    ...
    annotation(
      ...
      Documentation(info = "<html>
<p>This model describes a simple free-surface cylindrical liquid helium tank. The model is based on mass and energy balances, assuming that no heat transfer takes place except through the inlet and outlet flows. </p>
...
<li><i>1 Oct 2003</i> by <a href=\"mailto:francesco.casella@polimi.it\">Francesco Casella</a>:<br>First release.</li>
  ...
  model Flow1DFV "1-dimensional fluid flow model for helium liquid/gas (finite volumes)"
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFV2ph "1-dimensional fluid flow model for water/steam (finite volumes, 2-phase)"
    extends BaseClasses.Flow1DBase(redeclare replaceable package Medium = StandardWater "Medium model" constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium, FluidPhaseStart = Choices.FluidPhase.FluidPhases.TwoPhases);
    replaceable model HeatTransfer = Thermal.HeatTransferFV.IdealHeatTransfer constrainedby ThermoPower.Thermal.BaseClasses.DistributedHeatTransferFV annotation(
       choicesAllMatching = true);
    HeatTransfer heatTransfer(redeclare package Medium = Medium, final Nf = N, final Nw = Nw, final Nt = Nt, final L = L, final A = A, final Dhyd = Dhyd, final omega = omega, final wnom = wnom/Nt, final w = w*ones(N), final fluidState = fluidState) "Instantiated heat transfer model";
    ThermoPower.Thermal.DHTVolumes wall(final N = Nw) annotation(
...
//Friction factor selection
    ...
    for i in 1:N loop
      ...
      elseif FFtype == FFtype.Blasius then
        Cf[i] = f_blasius(w[i], Dhyd / A, e, Medium.dynamicViscosity(fluidState[integer(N / 2)])) * Kfc;      
      elseif FFtype == FFtypes.NoFriction then
        Cf[i] = 0;
      end if;
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFEM2ph "1-dimensional fluid flow model for water/steam (finite elements)"
    extends BaseClasses.Flow1DBase(Nw = N, redeclare replaceable package Medium = CryoHelium "Medium model" constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
    replaceable model HeatTransfer = Thermal.HeatTransferFEM.IdealHeatTransfer constrainedby ThermoPower.Thermal.BaseClasses.DistributedHeatTransferFEM annotation(
       choicesAllMatching = true);
    HeatTransfer heatTransfer(redeclare package Medium = Medium, final Nf = N, final Nw = Nw, final Nt = Nt, final L = L, final A = A, final Dhyd = Dhyd, final omega = omega, final wnom = wnom/Nt, final w = w, final fluidState = fluidState) "Instantiated heat transfer model";
    ThermoPower.Thermal.DHTNodes wall(N = N) annotation(
...
<li><i>1 Oct 2003</i>
  ...
  model Accumulator "Helium liqui-gas Accumulator"
    ...
    parameter SI.Volume Vl0 "LHe nominal volume (at reference level)";
    parameter SI.Area A "Cross Sectional Area";
    ...
    parameter SI.Pressure pg_start "Gas start pressure" annotation(
      Dialog(tab = "Initialisation"));
    parameter Units.AbsoluteTemperature Tg_start = 4.5 "Gas start temperature" annotation(
      Dialog(tab = "Initialisation"));
    parameter SI.CoefficientOfHeatTransfer gamma_ex = 50 "Liquid-Gas heat transfer coefficient";
    parameter SI.Temperature Tgin = 4.5 "Inlet gas temperature";
    parameter SI.MolarMass MM = 4e-3 "Gas molar mass";
    parameter SI.MassFlowRate wg_out0 "Nominal gas outlet flowrate";
    parameter Units.AbsoluteTemperature Tg0 = 4.5 "Nominal gas temperature";
    ...
    Modelica.Blocks.Interfaces.RealInput OutletValveOpening annotation(
      Placement(transformation(origin = {44, 90}, extent = {{-10, -10}, {10, 10}}, rotation = 270)));protected
  equation
...
<li><i>5 Feb 2004</i>
  ...
  model Drum "Drum for circulation boilers"
    ...
    parameter SI.HeatCapacity Cm = 0 "Total Heat Capacity of the metal wall" annotation(
      Evaluate = true);
    parameter SI.Temperature Text = 77 "External atmospheric temperature";
...
<li><i>1 Oct 2003</i>
  ...
  model DrumEquilibrium
    extends Icons.LHe.Drum;
    replaceable package Medium = CryioHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<p>It is possible to use a generic initial equation instead of <code>p = pstart</code>, e.g., to set the initial charge of a refrigeration circuit, by setting <code>initEquationFromInput = true</code> and connecting to the input the output of a <a href=\"Modelica://Modelica.Blocks.Sources.RealExpression\">Modelica.Blocks.Sources.RealExpression</a> component, containing the residual of the initial equation. In this case, if <code>useHomotopy = true</code>, the simplified initial equation is <code>(p - pstart)/pstart = 0</code>. To ensure a smooth homotopy transformation, it is recommended to write the residual in normalized form, i.e., with values have the order of magnitude of one, as for the simplified equation.</p>
  ...
  package BaseClasses "Contains partial models"
    ...
    partial model Flow1DBase "Basic interface for 1-dimensional water/steam fluid flow models"
      ...
      parameter Medium.AbsolutePressure pstart = 1e5 "Pressure start value" annotation(
        Dialog(tab = "Initialisation"));
      parameter Medium.SpecificEnthalpy hstartin = if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Liquid then -1e3 else if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Steam then 1e4 else 5e5 "Inlet enthalpy start value" annotation(
        Dialog(tab = "Initialisation"));
      parameter Medium.SpecificEnthalpy hstartout = if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Liquid then -1e3 else if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Steam then 1e4 else 5e5 "Outlet enthalpy start value" annotation(
        Dialog(tab = "Initialisation"));
...
<li><i>8 Oct 2004</i>
    ...
    partial model ValveBase "Base model for valves"
      ...
      parameter Medium.MassFlowRate wnom "Nominal mass flowrate" annotation(
        Dialog(group = "Nominal operating point"));
      parameter Medium.Density rhonom = 125 "Nominal density" annotation(
...
<li><i>1 Oct 2003</i>
    ...
    partial model PumpBase "Base model for centrifugal pumps"
      ...
      parameter Boolean use_in_Np = false "Use connector input for the pressure" annotation(
        Dialog(group = "External inputs"),
        choices(checkBox = true));
      parameter Units.LiquidDensity rho0 = 125 "Nominal Liquid Density" annotation(
        Dialog(group = "Characteristics"));
      parameter NonSI.AngularVelocity_rpm n0 "Nominal rotational speed" annotation(
        Dialog(group = "Characteristics"));
      parameter SI.Volume V = 0 "Pump Internal Volume" annotation(
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1Dfem2ph "1-dimensional fluid flow model for water/steam (finite elements)"
    ...
    import ThermoPower.Choices.Flow1D.FFtypes;
    import ThermoPower.Choices.Flow1D.HCtypes;
    extends BaseClasses.Flow1DBase(redeclare replaceable package Medium = StandardWater "Medium model" constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
    replaceable ThermoPower.Thermal.DHT wall(N = N) annotation(
      Dialog(enable = false),
      Placement(transformation(extent = {{-40, 40}, {40, 60}}, rotation = 0)));
    parameter Real alpha(min = 0, max = 2) = 1 "Numerical stabilization coefficient";
...
<li><i>1 Jul 2004</i>
  ...
  model SourceP "Pressure source for water/steam flows"
    ...
    parameter Units.HydraulicResistance R = 0 "Hydraulic resistance";
    parameter Medium.SpecificEnthalpy h = -1e3 "Nominal specific enthalpy";
    parameter Boolean allowFlowReversal = system.allowFlowReversal "= true to allow flow reversal, false restricts to design direction" annotation(
      Evaluate = true);
    outer ThermoPower.System system "System wide properties";
    Medium.AbsolutePressure p "Actual pressure";
...
<li><i>18 Mar 2005</i>
  ...
  package FMU "imported FMUs for external simulator imterface"
    ...
    protected
      ...
      annotation(
        Icon(coordinateSystem(preserveAspectRatio = false, initialScale = 0.1), graphics = {Rectangle(fillColor = {255, 255, 255}, lineThickness = 0.5, extent = {{-100, 100}, {100, -100}}), Text(textColor = {0, 4, 169}, extent = {{-100, -60}, {100, -100}}, textString = "%name"), Bitmap(extent = {{-95, -60}, {95, 95}}, fileName = "modelica://ThermoPower/Resources/Images/FMU.ico")}));
    end PartialFMU;
  end FMU;
  annotation(
    Documentation(info = "<HTML>
