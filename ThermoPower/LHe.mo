...
package LHe "Models of components with cryogenic helium as working fluid"
  connector Flange "Flange connector for cryogenic helium flows"
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model";
...
<li><i>1 Oct 2003</i>
  ...
  connector FlangeA "A-type flange connector for cryogenic helium flows"
    extends ThermoPower.LHe.Flange;
    annotation(
      Icon(graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {0, 128, 255}, fillPattern = FillPattern.Solid)}));
  end FlangeA;

  connector FlangeB "B-type flange connector for cryogenic helium flows"
    extends ThermoPower.LHe.Flange;
    annotation(
      Icon(graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 255}, fillColor = {0, 128, 255}, fillPattern = FillPattern.Solid), Ellipse(extent = {{-40, 40}, {40, -40}}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid)}));
  end FlangeB;
  ...
  model SourcePressure "Pressure source for cryogenic helium flows"
    ...
    parameter Medium.SpecificEnthalpy h = -1029.66 "Nominal specific enthalpy";
...
<li><i>1 Oct 2003</i>
  ...
  model SinkPressure "Pressure sink for cryogenic helium flows"
    extends Icons.LHe.SourceP;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    type HydraulicResistance = Real(final quantity = "HydraulicResistance", final unit = "Pa/(kg/s)");
...
<li><i>1 Oct 2003</i>
  ...
  model SourceMassFlow "Flowrate source for cryogenic helium flows"
    extends Icons.LHe.SourceW;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>1 Oct 2003</i>
  ...
  model SinkMassFlow "Flowrate sink for cryogenic helium flows"
    extends Icons.LHe.SourceW;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>18 Mar 2005</i>
  ...
  model PressDropLin "Linear pressure drop for cryogenic helium flows"
    extends Icons.LHe.PressDrop;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>1 Oct 2003</i>
  ...
  model PressDrop "Pressure drop for cryogenic helium flows"
    extends Icons.LHe.PressDrop;
...
<li><i>1 Oct 2003</i>
  ...
  model Header "Header with metal walls for cryogenic helium flows"
    extends Icons.LHe.Header;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    ...
    parameter Medium.AbsolutePressure pstart "Pressure start value" annotation(
      Dialog(tab = "Initialisation"));
    parameter Medium.SpecificEnthalpy hstart = if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Liquid then -1e3 else if FluidPhaseStart == Choices.FluidPhase.FluidPhases.Steam then 5e4 else 1e3 "Specific enthalpy start value" annotation(
      Dialog(tab = "Initialisation"));
    parameter Medium.Temperature Tmstart = 4.2 "Metal wall temperature start value" annotation(
      Dialog(tab = "Initialisation"));
...
// do nothing
    ...
    annotation(
      Icon(graphics),
      Documentation(info = "<HTML>
<p>This model describes a constant volume header with metal walls. The fluid can be He liquid, gas, or a two-phase mixture.
...
<li><i>1 Oct 2003</i>
  ...
  model Mixer "Mixer with metal walls for cryogenic helium flows"
    extends Icons.LHe.Mixer;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    Medium.ThermodynamicState fluidState "Thermodynamic state of the fluid";
...
// Set fluid properties
    ...
    dM_dt = V * (Medium.density_derp_h(fluidState) * der(p) + Medium.density_derh_p(fluidState) * der(h));
    dE_dt = h * dM_dt + M * der(h) - V * der(p);
    dM_dt = in1.m_flow + in2.m_flow + out.m_flow "Fluid mass balance";
    dE_dt = in1.m_flow * hi1 + in2.m_flow * hi2 + out.m_flow * ho - gamma * S * (T - Tm) + Q "Fluid energy balance";
    if Cm > 0 and gamma > 0 then
...
// do nothing
    ...
    annotation(
      Placement(transformation(extent = {{80, -20}, {120, 20}}, rotation = 0)),
      Documentation(info = "<HTML>
<p>This model describes a constant volume mixer with metal walls. The fluid can be He liquid, gas, or a two-phase mixture. The metal wall temperature and the heat transfer coefficient between the wall and the fluid are uniform. The wall is thermally insulated from the outside.
</HTML>", revisions = "<html>
<ul>
<li><i>23 May 2008</i>
...
// do nothing
    ...
    annotation(
      ...
      Documentation(info = "<HTML>
<p>This model describes the flow of liquid or gin a rigid tube. The basic modelling assumptions are:
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFV2ph "1-dimensional fluid flow model for cryogenic helium (finite volumes, 2-phase)"
    extends BaseClasses.Flow1DBase(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model", FluidPhaseStart = Choices.FluidPhase.FluidPhases.TwoPhases);
    replaceable model HeatTransfer = Thermal.HeatTransferFV.IdealHeatTransfer constrainedby ThermoPower.Thermal.BaseClasses.DistributedHeatTransferFV annotation(
       choicesAllMatching = true);
    HeatTransfer heatTransfer(redeclare package Medium = Medium, final Nf = N, final Nw = Nw, final Nt = Nt, final L = L, final A = A, final Dhyd = Dhyd, final omega = omega, final wnom = wnom / Nt, final w = w * ones(N), final fluidState = fluidState) "Instantiated heat transfer model";
    ThermoPower.Thermal.DHTVolumes wall(final N = Nw) annotation(
    ...
    constant Medium.AbsolutePressure pc = Medium.externalFluidConstants.criticalPressure;
    ...
    SI.Velocity u[N] "Fluid velocity";
    Medium.Temperature T[N] "Fluid temperature";
    Medium.Temperature Ts "Saturated liquid temperature";
...
// do nothing
    ...
    annotation(
      ...
      Icon(graphics = {Text(extent = {{-100, -40}, {100, -80}}, textString = "%name")}),
      Documentation(info = "<HTML>
<p>This model describes the flow of liquid or gas in a rigid tube. The basic modelling assumptions are:
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFEM "1-dimensional fluid flow model for cryogenic helium (finite elements)"
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DFEM2ph "1-dimensional fluid flow model for cryogenic helium (finite elements)"
    extends BaseClasses.Flow1DBase(Nw = N, redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model");
    replaceable model HeatTransfer = Thermal.HeatTransferFEM.IdealHeatTransfer constrainedby ThermoPower.Thermal.BaseClasses.DistributedHeatTransferFEM annotation(
       choicesAllMatching = true);
    HeatTransfer heatTransfer(redeclare package Medium = Medium, final Nf = N, final Nw = Nw, final Nt = Nt, final L = L, final A = A, final Dhyd = Dhyd, final omega = omega, final wnom = wnom / Nt, final w = w, final fluidState = fluidState) "Instantiated heat transfer model";
    ThermoPower.Thermal.DHTNodes wall(N = N) annotation(
...
<li><i>1 Oct 2003</i>
  ...
  model SensT "Temperature sensor for He liquid-gas"
    extends Icons.LHe.SensThrough;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    parameter Boolean allowFlowReversal = system.allowFlowReversal "= true to allow flow reversal, false restricts to design direction";
    outer ThermoPower.System system "System wide properties";
...
<li><i>1 Oct 2003</i>
  ...
  model SensT1 "Temperature sensor for cryogenic helium flows, single port"
    extends ThermoPower.Icons.LHe.SensP;
...
<li><i>1 Oct 2003</i>
  ...
  model SensW "Mass Flowrate sensor for cryogenic helium"
    extends Icons.LHe.SensThrough;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    parameter Boolean allowFlowReversal = system.allowFlowReversal "= true to allow flow reversal, false restricts to design direction";
...
<li><i>1 Oct 2003</i>
  ...
  model SensP "Pressure sensor for cryogenic helium flows"
    extends Icons.LHe.SensP;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>1 Oct 2003</i>
  ...
  model Accumulator "Liquid-Gas Accumulator"
    extends ThermoPower.Icons.LHe.Accumulator;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Liquid medium model" annotation(
       choicesAllMatching = true);
    Medium.ThermodynamicState liquidState "Thermodynamic state of the liquid";
    parameter SI.Volume V "Total volume";
    parameter SI.Volume Vl0 "Liquid nominal volume (at reference level)";
    parameter SI.Area A "Cross Sectional Area";
    parameter SI.Height zl0 "Height of liquid reference level over inlet/outlet connectors";
    parameter SI.Height zl_start "Liquid start level (relative to reference)" annotation(
      Dialog(tab = "Initialisation"));
    parameter Medium.SpecificEnthalpy hl_start "Liquid start specific enthalpy" annotation(
      Dialog(tab = "Initialisation"));
    ...
    parameter Units.AbsoluteTemperature Tg_start = 300 "Gas start temperature" annotation(
      Dialog(tab = "Initialisation"));
    parameter SI.CoefficientOfHeatTransfer gamma_ex = 50 "Liquid-Gas heat transfer coefficient";
    ...
    Medium.SpecificEnthalpy hl_in "He inlet specific enthalpy";
    Medium.SpecificEnthalpy hl_out "He outlet specific enthalpy";
    Medium.SpecificEnthalpy hl(start = hl_start, stateSelect = StateSelect.prefer) "He internal specific enthalpy";
    SI.Volume Vl "Volume occupied by LHe";
    SI.Mass Mg "Mass of gas";
    Medium.AbsolutePressure pf "He Pressure at the inlet/outlet flanges";
    SI.EnergyFlowRate Qp "LHe-Gas heat flow";
    ...
    Units.LiquidDensity rho "Density of the LHe";
    Modelica.Blocks.Interfaces.RealInput GasInfl annotation(
      Placement(transformation(extent = {{-84, 80}, {-64, 100}}, rotation = 0)));
    FlangeA LHeInfl(redeclare package Medium = Medium, m_flow(min = if allowFlowReversal then -Modelica.Constants.inf else 0)) annotation(
      Placement(transformation(extent = {{-44, -100}, {-24, -80}}, rotation = 0)));
    FlangeB LHeOutfl(redeclare package Medium = Medium, m_flow(max = if allowFlowReversal then +Modelica.Constants.inf else 0)) annotation(
...
// Boundary conditions
// (Thermal effects of the He going out of the accumulator are neglected)
    hl_in = homotopy(if not allowFlowReversal then inStream(LHeInfl.h_outflow) else if wl_in >= 0 then inStream(LHeInfl.h_outflow) else hl, inStream(LHeInfl.h_outflow));
    hl_out = homotopy(if not allowFlowReversal then hl else if wl_out >= 0 then inStream(LHeOutfl.h_outflow) else hl, hl);
    LHeInfl.h_outflow = inStream(LHeOutfl.h_outflow);
    LHeOutfl.h_outflow = inStream(LHeInfl.h_outflow);
    wl_in = LHeInfl.m_flow;
    wl_out = LHeOutfl.m_flow;
    LHeInfl.p = pf;
    LHeOutfl.p = pf;
    rho * A * der(zl) = wl_in + wl_out "He mass balance (density variations neglected)";
    rho * Vl * der(hl) - Vl * der(pg) - pg * der(Vl) = wl_in * (hl_in - hl) + wl_out * (hl_out - hl) - Qp "He energy balance";
...
// do nothing
    ...
    annotation(
      ...
      Icon(graphics),
      Documentation(info = "<HTML>
<p>This model describes a LHe-gas accumulator (the gas is modeled as ideal bi-atomic). <p>
He flows in and out through the interfaces at the component bottom (flow reversal supported). <p>
...
<li><tt>Vl0</tt>: volume occupied by He at the nominal level (reference value);
<li><tt>zl0</tt>: height of LHe nominal level above the LHe connectors;
<li><tt>A</tt>: cross-sectional area at actual LHe level;
</ul>
</HTML>", revisions = "<html>
<ul>
<li><i>30 May 2005</i>
...
<li><i>5 Feb 2004</i>
  ...
  model Drum2States
    extends Icons.LHe.Drum;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model" annotation(
       choicesAllMatching = true);
    ...
    parameter SI.Pressure pstart "Pressure start value" annotation(
      Dialog(tab = "Initialisation"));
    parameter SI.Volume Vldstart "Start value of drum LHe volume" annotation(
      Dialog(tab = "Initialisation"));
    parameter Choices.Init.Options initOpt = system.initOpt "Initialisation option" annotation(
      Dialog(tab = "Initialisation"));
    parameter Boolean noInitialPressure = false "Remove initial equation on pressure" annotation(
    ...
    SI.Mass Ml "Liquid He mass";
    SI.Mass Mv "Steam mass";
    SI.Mass M "Total liquid+steam mass";
    SI.Energy E "Total energy";
    SI.Volume Vt "Total volume";
    SI.Volume Vl(start = Vldstart + Vdcr) "Liquid He total volume";
    SI.Volume Vld(start = Vldstart, stateSelect = StateSelect.prefer) "Liquid He volume in the drum";
    SI.Volume Vv "Steam volume";
    Medium.AbsolutePressure p(start = pstart, stateSelect = StateSelect.prefer) "Drum pressure";
    Medium.MassFlowRate qf "Feed LHe mass flowrate";
    Medium.MassFlowRate qs "gas mass flowrate";
    SI.HeatFlowRate Q "Heat flow to the risers";
    Medium.SpecificEnthalpy hf "Feed LHe specific enthalpy";
...
<li><i>1 May 2004</i>
  ...
  model Drum "Drum for circulation boilers"
    ...
    parameter Real afd = 0.05 "Ratio of feed LHe in downcomer flowrate";
    ...
    SI.SpecificEnthalpy hf "Specific enthalpy of feedLHe";
    ...
    Medium.MassFlowRate wf "Mass flowrate of feedLHe";
    ...
    Medium.Temperature Ts "Saturated LHe temperature";
    ...
    FlangeA feedLHe(p(start = pstart), h_outflow(start = hlstart), redeclare package Medium = Medium, m_flow(min = if allowFlowReversal then -Modelica.Constants.inf else 0)) annotation(
...
// Boundary conditions
    feedLHe.p = p;
    feedLHe.m_flow = wf;
    feedLHe.h_outflow = hl;
    hf = homotopy(if not allowFlowReversal then inStream(feedLHe.h_outflow) else noEvent(actualStream(feedLHe.h_outflow)), inStream(feedLHe.h_outflow));
...
// do nothing
    ...
    annotation(
      ...
      Documentation(info = "<HTML>
<p>This model describes the cylindrical drum of a drum boiler, without assuming thermodynamic equilibrium between the liquid and vapour holdups. Connectors are provided for feed LHe inlet, steam outlet, downcomer outlet, riser inlet, and blowdown outlet.
...
<p>The enthalpy of the liquid going to the downcomer is computed by assuming that a fraction of the total mass flowrate (<tt>afd</tt>) comes directly from the feed LHe inlet. The pressure at the downcomer connector is equal to the vapour pressure plus the liquid head.
...
<li><i>1 Oct 2003</i>
  ...
  model ValveLin "Valve for cryogenic helium flows with linear pressure drop"
    extends Icons.LHe.Valve;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
...
<li><i>1 Oct 2003</i>
  ...
  model ValveLiq "Valve for liquid He flow"
  ...
  equation
    ...
    annotation(
      ...
      Diagram(graphics),
      Documentation(info = "<HTML>
<p>Liquid He valve model according to the IEC 534/ISA S.75 standards for valve sizing, incompressible fluid. <p>
...
<li><i>1 Oct 2003</i>
  ...
  model ValveVap "Valve for steam flow"
    extends BaseClasses.ValveBase(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
    import ThermoPower.Choices.Valve.CvTypes;
...
// Dummy values
  ...
  equation
    ...
    annotation(
      ...
      Diagram(graphics),
      Documentation(info = "<HTML>
<p>Liquid He valve model according to the IEC 534/ISA S.75 standards for valve sizing, compressible fluid. <p>
Extends the <tt>ValveBase</tt> model (see the corresponding documentation for common valve features).
...
<li><i>1 Oct 2003</i>
  ...
  model ValveLiqChoked "Valve for liquid He flow, allows choked flow conditions"
    extends BaseClasses.ValveBase(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
    import ThermoPower.Choices.Valve.CvTypes;
  ...
  equation
    ...
    Ff = 0.96 - 0.28 * sqrt(pv / Medium.externalFluidConstants.criticalPressure);
    ...
    annotation(
      ...
      Diagram(graphics),
      Documentation(info = "<HTML>
<p>Liquid He valve model according to the IEC 534/ISA S.75 standards for valve sizing, incompressible fluid, with possible choked flow conditions. <p>
...
<li><i>15 Dec 2003</i>
  ...
  model PumpNPSH
    extends Pump(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium);
...
<li><i>20 Dec 2003</i>
  ...
  function f_colebrook "Fanning friction factor for cryogenic helium flows"
...
<li><i>1 Oct 2003</i>
  ...
  function f_colebrook_2ph "Fanning friction factor for a two phase cryogenic helium flow"
    input SI.MassFlowRate w;
    input Real D_A;
    input Real e;
...
<li><i>1 Oct 2003</i>
  ...
  equation
    w = homotopy(Kt * partialArc * sqrt(Medium.pressure(steamState_in) * Medium.density(steamState_in)) * Functions.sqrtReg(1 - (1 / PR) ^ 2), partialArc / partialArc_nom * wnom / pnom * Medium.pressure(steamState_in)) "Stodola's law";
...
<li><i>20 Apr 2005</i>
  ...
  model SteamTurbineUnit "Simplified HP+LP turbine unit for steam flows"
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
       choicesAllMatching = true);
    extends Icons.LHe.SteamTurbineUnit;
...
<p>It is possible to use a generic initial equation instead of <code>p = pstart</code>, e.g., to set the initial charge of a refrigeration circuit, by setting <code>initEquationFromInput = true</code> and connecting to the input the output of a <a href=\"Modelica://Modelica.Blocks.Sources.RealExpression\">Modelica.Blocks.Sources.RealExpression</a> component, containing the residual of the initial equation. In this case, if <code>useHomotopy = true</code>, the simplified initial equation is <code>(p - pstart)/pstart = 0</code>. To ensure a smooth homotopy transformation, it is recommended to write the residual in normalized form, i.e., with values have the order of magnitude of one, as for the simplified equation.</p>
  ...
  package BaseClasses "Contains partial models"
    ...
    partial model Flow1DBase "Basic interface for 1-dimensional cryogenic helium fluid flow models"
      replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model" annotation(
         choicesAllMatching = true);
      extends Icons.LHe.Tube;
      constant Real pi = Modelica.Constants.pi;
...
<li><i>8 Oct 2004</i>
    ...
    partial model ValveBase "Base model for valves"
      ...
      parameter SI.Pressure dpnom "Nominal pressure drop" annotation(
        Dialog(group = "Nominal operating point"));
      parameter Medium.MassFlowRate wnom "Nominal mass flowrate" annotation(
        Dialog(group = "Nominal operating point"));
      parameter Medium.Density rhonom = 130 "Nominal density" annotation(
...
<ul><li><tt>CvData = ThermoPower.LHe.ValveBase.CvTypes.Av</tt>: the flow coefficient is given by the metric <tt>Av</tt> coefficient (m^2).
<li><tt>CvData = ThermoPower.LHe.ValveBase.CvTypes.Kv</tt>: the flow coefficient is given by the metric <tt>Kv</tt> coefficient (m^3/h).
<li><tt>CvData = ThermoPower.LHe.ValveBase.CvTypes.Cv</tt>: the flow coefficient is given by the US <tt>Cv</tt> coefficient (USG/min).
<li><tt>CvData = ThermoPower.LHe.ValveBase.CvTypes.OpPoint</tt>: the flow coefficient is specified by the nominal operating point:  <tt>pnom</tt>, <tt>dpnom</tt>, <tt>wnom</tt>, <tt>rhonom</tt>, <tt>thetanom</tt> (in forward flow).
</ul>
...
// The next equation is provided to close the balance but never actually used
      inlet.h_outflow = outlet.h_outflow;
      annotation(
        Icon(graphics = {Polygon(fillColor = {0, 128, 255}, fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{-28, 76}, {-28, 28}, {-22, 28}, {-22, 82}, {-60, 82}, {-60, 76}, {-28, 76}}), Polygon(fillColor = {0, 128, 255}, fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{26, 56}, {32, 56}, {32, 76}, {60, 76}, {60, 82}, {26, 82}, {26, 56}}), Rectangle(fillColor = {160, 160, 164}, fillPattern = FillPattern.Sphere, extent = {{-60, 8}, {60, -8}}), Polygon(fillColor = {0, 128, 255}, fillPattern = FillPattern.Solid, lineThickness = 0.5, points = {{-28, 28}, {-28, -26}, {32, -60}, {32, 60}, {-28, 28}}), Text(extent = {{-130, -60}, {128, -100}}, textString = "%name")}, coordinateSystem(initialScale = 0.1)),
        Diagram(graphics),
        Documentation(info = "<html>
...
<li><i>5 Oct 2011</i>
    ...
    partial model EvaporatorBase "Basic interface for 1-dimensional cryogenic helium fluid flow models"
      replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model";
      ...
      parameter Medium.SpecificEnthalpy hstartin = 1e3 "Inlet enthalpy start value";
      parameter Medium.SpecificEnthalpy hstartout = 1e3 "Outlet enthalpy start value";
    ...
    equation
      ...
      Eb = Mbl * hls + Mbv * hvs - p * Vb "Energy of boiling LHe";
...
This model is not yet complete
  ...
  model Flow1D "1-dimensional fluid flow model for cryogenic helium (finite volumes)"
    extends BaseClasses.Flow1DBase;
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1DDB "1-dimensional fluid flow model for liquid or gas flow (finite volumes, Dittus-Boelter heat transfer)"
...
<p>This model can be used in the case of one-phase liquid or gas flow. Dittus-Boelter's equation [1] is used to compute the heat transfer coefficient.
<p><b>References:</b></p>
<ol>
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1D2ph "1-dimensional fluid flow model for cryogenic helium (finite volumes, 2-phase)"
    extends Modelica.Icons.ObsoleteModel;
    extends BaseClasses.Flow1DBase(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model", FluidPhaseStart = Choices.FluidPhase.FluidPhases.TwoPhases);
    replaceable ThermoPower.Thermal.DHT wall(N = N) annotation(
      Dialog(enable = false),
...
<p>This simplified model can be used for one-phase or two-phase cryogenic helium flow. The heat transfer coefficient is computed according to the following hypotheses:
<ul>
...
<li><i>1 Oct 2003</i>
  ...
  equation
    hCHF = hl + xCHF * (hv - hl) "Specific enthalpy corresponding to critical heat flux";
// Saturated liquid and gas properties
    bubble = Medium.setBubbleState(sat, 1);
    dew = Medium.setDewState(sat, 1);
    mu_ls = Medium.dynamicViscosity(bubble);
    k_ls = Medium.thermalConductivity(bubble);
...
// to achieve continuous h.t.c.
    ...
    annotation(
      Documentation(info = "<HTML>
<p>This model extends <tt>Flow1D2ph</tt> by computing the distribution of the heat transfer coefficient <tt>gamma</tt> and making it available through an extended version of the <tt>wall</tt> connector.
<p>The model can be used for one-phase or two-phase cryogenic helium flow. The heat transfer coefficient is computed according to the following hypotheses:
<ul>
<li> If the fluid is subcooled liquid or superheated steam, Dittus-Boelter's correlation [1] is used.
...
<li><i>1 Oct 2003</i>
  ...
  model Flow1Dfem2ph "1-dimensional fluid flow model for cryogenic helium (finite elements)"
    ...
    import ThermoPower.Choices.Flow1D.FFtypes;
    import ThermoPower.Choices.Flow1D.HCtypes;
    extends BaseClasses.Flow1DBase(redeclare replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialTwoPhaseMedium "Medium model");
    replaceable ThermoPower.Thermal.DHT wall(N = N) annotation(
      Dialog(enable = false),
      Placement(transformation(extent = {{-40, 40}, {40, 60}}, rotation = 0)));
    parameter Real alpha(min = 0, max = 2) = 1 "Numerical stabilization coefficient";
    ...
    constant SI.Pressure pzero = 10 "Small deltap for calculations";
    constant SI.Pressure pc = Medium.externalFluidConstants.criticalPressure;
    constant SI.SpecificEnthalpy hzero = 1e-4;
    Medium.ThermodynamicState fluidState[N] "Thermodynamic state of the fluid at the nodes";
    Medium.SaturationProperties sat "Properties of saturated fluid";
    Medium.ThermodynamicState dew "Thermodynamic state at dewpoint";
    Medium.ThermodynamicState bubble "Thermodynamic state at bubblepoint";
...
<li><i>1 Jul 2004</i>
  ...
  model SourceP "Pressure source for cryogenic helium flows"
    extends Icons.LHe.SourceP;
    extends Modelica.Icons.ObsoleteModel;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model";
...
<li><i>1 Oct 2003</i>
  ...
  model SinkP "Pressure sink for cryogenic helium flows"
    extends Icons.LHe.SourceP;
    extends Modelica.Icons.ObsoleteModel;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model";
    parameter Medium.AbsolutePressure p0 = 1.01325e5 "Nominal pressure";
...
<li><i>1 Oct 2003</i>
  ...
  model SourceW "Flowrate source for cryogenic helium flows"
    extends Icons.LHe.SourceW;
    extends Modelica.Icons.ObsoleteModel;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model";
...
<li><i>1 Oct 2003</i>
  ...
  model SinkW "Flowrate sink for cryogenic helium flows"
    extends Icons.LHe.SourceW;
    extends Modelica.Icons.ObsoleteModel;
    replaceable package Medium = CryoHelium constrainedby Modelica.Media.Interfaces.PartialMedium "Medium model";
    parameter Medium.MassFlowRate w0 = 0 "Nominal mass flowrate";
